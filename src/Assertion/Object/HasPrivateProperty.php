...
final class HasPrivateProperty implements Assertion
{
    ...
    public function assert(mixed $value, string $message = ''): void
    {
        if (! $this($value)) {
            $message = sprintf(
                ! (new IsEmpty())($message) ?
                    $message : 'Expected %s to have a private property named %2$s.',
    ...
    public function __invoke(mixed $value): bool
    {
        ...
        return $reflector->hasProperty($this->property)
            && $reflector->getProperty($this->property)->isPrivate();
    }
}
